...
class BoardBattleBot(Board):
    ...
    def __init__(self, screen):
        ...
        self.player = AvatarScore(assets.assets.avatar_path, (self.board_center_x - 290, self.board_y - 40), BoardBattleBot.score_player, self.font)
        self.bot = AvatarScore(assets.assets.avatar_path, (self.board_center_x + 100, self.board_y - 40), BoardBattleBot.score_player, self.font, "bot")
        self.bot_action = Bot()

    # def bot(self):
    #     return self.bot_action.update(self.tiles)

    def draw(self):
        super().draw()
        self.player.draw(self.screen)
        self.player.update_score(BoardBattleBot.score_player)

        self.bot.draw(self.screen)
        self.bot.update_score(BoardBattleBot.score_bot)
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not BoardBattleBot.game_over:  # Turn on or turn off volume
            ...
            for btn in self.restart_back.buttons:
                if btn.btn["rect"].collidepoint(event.pos):
                    ...
                    sound.sound.Sound.play_music(config.CLICK)
                    BoardBattleBot.score_player = 0
                    BoardBattleBot.score_bot = 0
                    ...
                    if self.tiles[row][col] is not None and self.tiles[row][col].rect.collidepoint(event.pos):
                        ...
                        if self.first_tile is None:
                            ...
                            if path is not None:
                                ...
                                BoardBattleBot.score_player += BoardBattleBot.score
